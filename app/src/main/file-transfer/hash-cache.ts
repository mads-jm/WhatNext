    ...
    async init(): Promise<void> {
        try {
            ...
            if ((err as NodeJS.ErrnoException).code !== 'ENOENT') {
                console.warn('[HashCache] Could not parse hashes.json — starting fresh:', err)
            }
            this.index = {}
        }
    }
    ...
    private async persist(): Promise<void> {
        ...
        try {
            ...
            try {
                ...
                console.warn(`[HashCache] Atomic rename failed, leaving tmp file at ${tmpPath}:`, renameErr)
            }
        } finally {
