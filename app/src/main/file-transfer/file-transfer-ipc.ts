 ...
 */
function sanitizeFilename(filename: string, sha256: string): string {
...
/**
 * Assert the resolved path is strictly inside targetDir.
 * Throws if the path escapes — call site should abort the transfer.
 */
function assertPathContained(resolvedPath: string, targetDir: string): void {
    ...
    if (!startsWith(normalised, resolvedTarget + path.sep) && !cmp(normalised, resolvedTarget)) {
        throw new Error(
