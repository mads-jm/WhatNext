...
const trackFileMap = new Map<
    string,
    { audioPath?: string; artworkPath?: string }
>();
...
const speedTrackers = new Map<
    string,
    { startMs: number; startBytes: number }
>();
...
function processQueue(): void {
    ...
    while (i < downloadQueue.length) {
        ...
        if (isAudio) {
            ...
            if (
                activeArtworkCount >=
                FILE_TRANSFER_CONFIG.MAX_CONCURRENT_ARTWORK
            ) {
                i++;
                continue;
            }
...
async function _doPersistTransferState(): Promise<void> {
    ...
    const statePath = path.join(
        partialDir,
        FILE_TRANSFER_CONFIG.TRANSFER_STATE_FILE,
    );
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (Number.isNaN(age) || age < cutoff) {
                console.log(
                    '[FileTransfer] Discarding stale transfer (>24h):',
                    entry.sha256,
                );
                continue;
            }
        ...
        console.log(
            `[FileTransfer] Loaded ${activeTransfers.size} transfer(s) from disk`,
        );
    } catch (err: unknown) {
        if ((err as NodeJS.ErrnoException).code !== 'ENOENT') {
            console.warn(
                '[FileTransfer] Could not parse transfers.json — starting fresh:',
                err,
            );
        }
    ...
    for (const entry of entries) {
        ...
        if (!activeTransfers.has(sha256)) {
            ...
            try {
                ...
                console.log(
                    '[FileTransfer] Cleaned up orphan partial file:',
                    entry,
                );
            } catch (err) {
                console.warn(
                    '[FileTransfer] Could not delete stale partial:',
                    entry,
                    err,
                );
            }
...
export async function registerFileTransferHandlers(
        ...
        async (
            ...
            for (const file of files) {
                ...
                if (hashCache && hashCache.getPathBySha256(file.sha256)) {
                    ...
                    const localFilePath = hashCache.getPathBySha256(
                        file.sha256,
                    )!;
    ...
    ipcMain.handle(
        ...
        (
            _e,
            { playlistId, enabled }: { playlistId: string; enabled: boolean },
        ) => {
...
export function handleFileTransferUtilityMessage(message: IPCMessage): boolean {
    switch (message.type) {
        ...
        case UtilityToMainMessageType.FILE_TRANSFER_MANIFEST_RECEIVED: {
            const { manifest } = message.payload as {
                requestId: string;
                peerId: string;
...
async function handleManifestRequest(payload: {
    ...
    if (!utility) {
        console.warn(
            '[FileTransfer] Cannot send manifest: utility process not running',
        );
        return;
    }
    ...
    if (!hashCache) {
        console.warn(
            '[FileTransfer] Cannot build manifest: hash cache not initialised',
        );
        return;
    }
...
async function handleIncomingRequest(payload: {
    ...
    if (!cache) {
        ...
        console.warn(
            '[FileTransfer] Serve request arrived before initialisation — refusing',
        );
    ...
    if (!utility) {
        console.warn(
            '[FileTransfer] Cannot serve: utility process not running',
        );
        return;
    }
...
async function handleChunkReceived(payload: {
    ...
    if (transfer) {
        ...
        transfer.bytesReceived = Math.max(
            transfer.bytesReceived,
            verdict.bytesReceived,
        );
        transfer.status = 'transferring';
...
async function handleTransferComplete(payload: {
    ...
    if (!isValidSha256(sha256)) {
        console.error(
            '[FileTransfer] handleTransferComplete: invalid sha256 rejected:',
            sha256,
        );
    ...
    if (!transfer) {
        console.warn(
            '[FileTransfer] complete received for unknown transfer:',
            sha256,
        );
        return;
    }
    ...
    if (actualHash !== sha256) {
        ...
        failTransfer(
            sha256,
            `Hash mismatch: expected ${sha256}, got ${actualHash}`,
        );
        return;
    }
...
async function abortTransfer(
    ...
    if (utility) {
        utility.postMessage(
            createIPCMessage(MainToUtilityMessageType.FILE_TRANSFER_CANCEL, {
                peerId,
                sha256,
            }),
        );
    ...
    await fs.promises
        .unlink(path.join(partialDir, `${sha256}.tmp`))
        .catch(() => undefined);
}
...
async function resolveCollision(filePath: string): Promise<string> {
    ...
    throw new Error(
        `Too many filename collisions for '${path.basename(filePath)}'`,
    );
}
