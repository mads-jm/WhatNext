    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 bg-surface-high/50 rounded-lg p-3">
                ...
                >
                    <i
                        className={`fa-solid ${resetting ? 'fa-spinner fa-spin' : 'fa-database'}`}
                    />
                    {resetting ? 'Resetting...' : 'Reset Database'}
                </button>
                <span className="text-xs text-on-surface-variant">
                    Clears IndexedDB and reinitializes — use after schema
                    changes
                </span>
                {resetStatus && (
                    <span
                        className={`text-xs ml-auto ${resetStatus.includes('failed') ? 'text-error' : 'text-primary'}`}
                    >
                        {resetStatus}
                    </span>
                )}
            ...
            {activeTab === 'protocols' && (
                <div className="space-y-3">
                    {PROTOCOLS.map((proto) => (
                        <div
                            key={proto.name}
                            className="bg-surface-high rounded-lg p-4 flex items-start gap-4"
                        >
                            <div className="w-10 h-10 rounded-lg bg-purple-900/40 flex items-center justify-center flex-shrink-0">
                                <i
                                    className={`${proto.icon} text-purple-400`}
                                />
                            </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-on-surface-variant mb-3">
                                    Testing utilities for playlist sync and file
                                    transfer.
                                </p>
                                ...
                                <p className="text-xs text-on-surface-variant mt-2 italic">
                                    Connect to a peer via the Network Status
                                    panel to enable testing.
                                </p>
                            </div>
                        </div>
                    </div>
