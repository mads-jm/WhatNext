...
export function P2PStatus() {
    ...
    const nodeStatusColor = status.nodeStarted
        ? 'bg-primary'
        ...
        <div className="space-y-4 font-mono text-sm">
            ...
            <div className="bg-surface-high rounded-lg p-4 border-l-4 border-primary">
                ...
                <p className="text-xs text-on-surface-variant">
                    Your node, your rules &mdash; direct peer connections with
                    no intermediary
                </p>
            </div>
            ...
            <P2PSection
                ...
                badge={
                    <div
                        className={`w-3 h-3 rounded-full ${nodeStatusColor}`}
                    />
                }
            >
                        ...
                        <>
                            ...
                            {status.multiaddrs.length > 0 && (
                                <div>
                                    <span className="text-on-surface-variant font-semibold text-xs">
                                        Listening Addresses (
                                        {status.multiaddrs.length}):
                                    </span>
                                    <div className="mt-1 space-y-1">
                                        {status.multiaddrs.map((addr) => (
                                            <div
                                                key={addr}
                                                className="bg-surface-high rounded px-2 py-1 text-xs break-all"
                                            >
                                                {addr}
                                            </div>
                                        ))}
            ...
            >
                {status.discoveredPeers.length === 0 ? (
                    <p className="text-xs text-on-surface-variant italic">
                        No peers discovered. Ensure another WhatNext instance is
                    ...
                    <div className="space-y-2">
                        {status.discoveredPeers.map((peer) => (
                            <P2PPeerCard
                                key={peer.peerId}
                                peer={peer}
                                isConnected={status.connectedPeers.includes(
                                    peer.peerId,
                                )}
                                onConnect={() => p2p.handleConnect(peer.peerId)}
                                onDisconnect={() =>
                                    p2p.handleDisconnect(peer.peerId)
                                }
                                onSelectDetails={() =>
                                    p2p.openPeerDetails(peer)
                                }
                            />
                        ))}
                    </div>
            ...
            {p2p.selectedPeer && (
                ...
                >
                    <P2PPeerDetails
                        peer={p2p.selectedPeer}
                        onClose={p2p.closePeerDetails}
                    />
                </P2PSection>
            )}
