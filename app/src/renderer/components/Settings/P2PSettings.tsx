...
export function P2PSettings() {
    ...
    const [connectedRelayAddr, setConnectedRelayAddr] = useState<string | null>(
        null,
    );
    ...
    useEffect(() => {
        const cleanup = window.electron?.p2p.onRelayStatus((status) => {
            setConnectedRelayAddr(
                status.connected ? status.relayMultiaddr : null,
            );
            setRelays((prev) =>
                prev.map((r) => ({
                    ...r,
                    connected:
                        status.connected && r.address === status.relayMultiaddr,
                })),
            );
        });
        return () => {
    ...
    const handleAdd = async () => {
        ...
        if (!trimmed.startsWith('/')) {
            setAddError(
                'A relay address must be a multiaddr starting with "/" (e.g. /ip4/1.2.3.4/tcp/4001/p2p/12D3Koo…)',
            );
            return;
        }
    ...
    return (
        <div className="p-6 max-w-2xl">
            ...
            <p className="text-sm text-on-surface-variant mb-6">
                Relay servers enable WhatNext sessions between friends on
                different networks. Your session data routes through the relay
                you choose — WhatNext does not control this. You can{' '}
                <button
                    className="text-primary underline cursor-pointer"
                    onClick={() =>
                        window.electron?.shell.openExternal(
                            'https://github.com/mads-jm/WhatNext/tree/main/relay',
                        )
                    }
                >
                    self-host the relay
            ...
            <div className="space-y-2 mb-6">
                {relays.length === 0 && (
                    <div className="text-sm text-on-surface-variant py-3 text-center border border-dashed border-outline-variant rounded-lg">
                        No relay servers configured. Add one below to enable
                        remote sessions.
                    </div>
                )}
                {relays.map((relay) => (
            ...
            <div className="space-y-2">
                ...
                <p className="text-xs text-on-surface-variant">
                    Paste the multiaddr shown when you start the relay server
                    (includes /p2p/&lt;PeerID&gt;).
                </p>
            </div>
        </div>
