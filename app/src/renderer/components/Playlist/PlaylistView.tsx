...
function PlaylistHeader({ playlist, trackCount, totalDuration, onAddTracks }: PlaylistHeaderProps) {
    ...
    const handleExport = (format: 'markdown' | 'html') => {
        exportAndSave(playlist.id, format);
        setExportOpen(false);
    };
...
interface PlaylistTrackListProps {
    ...
    playlistId: string;
    expandedTrackComments: string | null;
    onToggleComments: (trackId: string) => void;
    onContextMenu: (e: React.MouseEvent, items: ContextMenuItem[]) => void;
    buildMenuItems: (track: TrackViewModel) => ContextMenuItem[];
}

function PlaylistTrackList({
    tracks,
    playlistId,
    expandedTrackComments,
    onToggleComments,
    onContextMenu,
    buildMenuItems,
...
export function PlaylistView({ playlistId }: PlaylistViewProps) {
    ...
    const buildTrackMenuItems = (track: {
        ...
        if (track.purchaseLinks && track.purchaseLinks.length > 0) {
