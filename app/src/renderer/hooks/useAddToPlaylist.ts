...
export function useAddToPlaylist() {
    const [addingTo, setAddingTo] = useState<{ trackId: string; playlistId: string } | null>(null);
    const [feedback, setFeedback] = useState<string | null>(null);
    const timerRef = useRef<ReturnType<typeof setTimeout>>(undefined);

    const add = useCallback(async (trackId: string, playlistId: string, playlistName: string) => {
