...
**Do not include `Co-Authored-By` lines in commits.** If Codex contributed, note it in the PR description instead. All commits are human-owned.
...
- **Worktree cleanup is Codex's responsibility**: after every agentic session, run `git worktree remove <path> && git branch -d <branch>` before closing.
