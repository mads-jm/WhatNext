...
Then, first tickets after merge: [[epic-session-liveness-fixes|companion auth/host-secret]] (§2), the `useTrackSource` syncingRef deadlock (§2), [[epic-file-transfer-guards|the cancel-path slot leak]] (§2), the local-only playback mutex (§2 — or relabel the UI), `set-backend-path` validation (§2), and [[epic-quality-gates|the typecheck/lint gate repair]] (§2).
