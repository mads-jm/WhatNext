...
**issue (✅ closed 2026-08-04):** `app/src/main/file-transfer/file-transfer-ipc.ts:512-536` — **Cancel leaks a concurrency slot.** `FILE_TRANSFER_CANCEL` sets status `cancelled` and closes the handle but never decrements `activeAudioCount`/`activeArtworkCount` nor calls `processQueue()` (compare the complete path at `:988-996` and `failTransfer` at `:1126-1133`). With `MAX_CONCURRENT_AUDIO = 1`, a single cancelled audio transfer stalls the entire audio queue until some unrelated transfer completes or errors. **Closed** with punch-list item 5: slot ownership is now recorded when `processQueue` dispatches and released exactly once by whichever of complete/fail/cancel gets there first, so cancel kicks the queue and a late complete-after-cancel cannot over-release. Correction to this review's framing: the complete/fail pair was *already* able to double-release (and to release a slot a queued-and-failed transfer never took) — cancel would have been a third caller of a broken decrement, so the fix is ownership tracking rather than a fourth copy of the decrement.
...
**issue (✅ closed 2026-08-05):** `app/src/main/file-transfer/file-transfer-ipc.ts:802-830` — **Serving by sha256 ignores `sharingState`.** Manifests are deny-by-default (nice — see [[#Worth keeping]]), but `handleIncomingRequest` serves any sha256 present in the hash cache regardless of whether sharing is enabled, and disabling sharing doesn't revoke previously-learned hashes. Hash entropy makes blind guessing impractical, but the authorization model is "knowing the name of the file" — check `sharingState` (or a served-hashes allowlist built at manifest time) before streaming. **Closed** with the second of the two options, ruled by the user 2026-08-05: `serve-guards.ts` holds a served-hashes allowlist recorded when a manifest is built under active sharing and revoked when sharing is toggled off, consulted before any `stat` or read. Refusal is byte-identical to not-found so it cannot be used as an enumeration oracle. Widening to this review's framing: because `HashCache` persists and `register()` adds every file *received* from a peer, the exposed set was not just the current session's playlist but everything this install had ever hashed. One accepted limitation ships with it — served files fail closed after a host restart until a manifest is re-fetched; see [[epic-file-transfer-guards]] *Risks & Open Questions*.
...
> ✅ **Closed 2026-08-05** ([[epic-quality-gates]] cycles 1 & 2) — typecheck green in cycle 1, lint green in cycle 2. All three gates green locally; **CI green is still unobserved** (first observation is when the wave-2 PR opens).
...
> - **Lint was untouched in cycle 1 and still red** (re-measured at mvp `413e6bf`: **55 errors / 16 warnings**, not 59). Deferred to cycle 2 on purpose — a ~28-file sweep would collide with the in-flight `receive-path-lifecycle` lane. Consequence: CI showed one red check (Lint) by design until that landed.
>
> **Lint: green 2026-08-05 (cycle 2) — 0 errors, warnings 16 → 12.** The baseline re-measured at `d5b44fa` as exactly 55/16; the dominant classes were `no-explicit-any` (32) and `no-unused-vars` (16). This review's "burn this down or the gate stays decorative" is discharged, with two things worth carrying forward:
> - **The same build-state bug this review's typecheck half turned up applies to lint.** `globalIgnores(['dist'])` was ESLint's only project ignore, so `release/`, `playwright-report/` and `test-results/` were linted whenever they existed — probed causally at 55 → 58 errors. `release/` is the sharp one: electron-builder vendors `node_modules` there and every `.d.ts` matches `**/*.ts`. Fixed in the ignore list. **Worth generalising: "does this gate's result depend on what is on disk?" found a real defect in two of the three gates.**
> - **Warnings are 12, not 0, deliberately.** All 12 are `react-hooks/exhaustive-deps`. Driving them to zero changes when effects re-run — a behaviour change by construction — so it is recorded as its own follow-up in [[epic-quality-gates]] rather than smuggled into a no-op sweep. The runtime-no-op claim was verified by diffing built `dist/` output against `d5b44fa`, not by inspection: the utility and preload bundles are byte-identical, `main.js` differs by one line and the renderer bundle by one hunk.
...
| `npm run lint` | ⚠️ → ✅ **2026-08-05** | Config fixed by lint lane; errors were pre-existing debt (`no-explicit-any`, unused vars) across ~28 files. *Re-measured at mvp `413e6bf`: **55 / 16**, and again at `d5b44fa`: **55 / 16**. Burned down by [[epic-quality-gates]] cycle 2 to **0 errors / 12 warnings** — warnings are all `react-hooks/exhaustive-deps` and are deliberately not zero (fixing them changes effect re-run timing; own follow-up). Like typecheck, the count depended on build state — `release/`, `playwright-report/`, `test-results/` were gitignored but not ESLint-ignored; now in `globalIgnores`* |
| `npm run typecheck` | ❌ → ✅ **2026-08-05** | All failures environmental (node_modules `.d.ts` + `vite.config.ts` under current `moduleResolution`); zero source errors — re-measured at mvp `413e6bf` as **16** errors (15 with no renderer build on disk; the config's `allowJs` + missing `include` made the count depend on build state) and **confirmed zero in `src/`**. Now 0; see §2 |
...
Then, first tickets after merge: [[epic-session-liveness-fixes|companion auth/host-secret]] (§2), the `useTrackSource` syncingRef deadlock (§2), ~~[[epic-file-transfer-guards|the cancel-path slot leak]] (§2)~~ (pulled forward and closed 2026-08-04 with item 5), the local-only playback mutex (§2 — or relabel the UI), ~~`set-backend-path` validation (§2)~~ (pulled forward and closed 2026-08-04 with item 4), and ~~[[epic-quality-gates|the typecheck/lint gate repair]] (§2)~~ (closed 2026-08-05 across cycles 1 and 2).
